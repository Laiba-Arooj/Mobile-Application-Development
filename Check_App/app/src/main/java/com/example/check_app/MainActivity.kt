    ...
    private fun getSentiment() {
        ...
        if (twtlink != "") {
            ...
            val stringRequest = StringRequest(Request.Method.GET,
                "https://twtsentiment-fastapi.herokuapp.com/twt_link?link=$twtlink",
                    Response.Listener<String> { response ->
                        val obj = JSONObject(response)
                        Log.d("Response", response.toString())
                            txtOutput.text = obj.getString("sentence")
                            txtEmotion.text = obj.getString("prediction")
                    },
                Response.ErrorListener { txtOutput.text = "That didn't work!" })
